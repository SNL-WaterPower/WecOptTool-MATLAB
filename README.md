...
temporary folder associated to the `RM3Study` object and are deleted upon 
destruction of the object. If you wish to retrieve these files prior to 
deletion, use the `saveData` method of the `RM3Study` object as follows: 

```matlab
study = RM3Study()                       % Create the RM3Study object
...                                      % Set up the study (omitted)
run(study)                               % Execute the study
study.saveData(/path/to/save/folder)     % Save the data to a given path
```
