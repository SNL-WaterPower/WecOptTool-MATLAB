...
A number of files are generated by WecOptTool to handle inputs and outputs, 
particularly for the NEMOH hydrodynamic solver. These files are stored within a 
temporary folder associated to the `Blueprint` subclass object and are deleted upon 
destruction of the object. If you wish to retrieve these files prior to 
deletion, use the `saveData` method, as shown here for the RM3 example: 

```matlab
blueprint = RM3()                           % Create the RM3Study object
 ...                                        % Run some cases
blueprint.saveData(/path/to/save/folder)    % Save the data to a given path
```
...
1. **WecOptTool** provides the user interface to the toolbox. The main 
   components are: 
     * The `Blueprint` class, used for creating new WEC models.
     * The `Device` class, whose objects are created by Blueprints
       and are used to simulate a particular device configuration
2. **WecOptLib** contains additional business logic for testing and 
   implementing the WecOptTool package.
